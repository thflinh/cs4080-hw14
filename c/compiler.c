...
typedef struct ClassCompiler {
  ...
  uint8_t currentMethodNameConstant;
  bool inMethodBody;
} ClassCompiler;
//< Methods and Initializers class-compiler-struct
...
//> Methods and Initializers parse-call
    ...
    uint8_t argCount = argumentList();
    int cacheIndex = addInvokeCache(currentChunk());
    emitBytes(OP_INVOKE, name);
    emitByte(argCount);
    emitByte((uint8_t)cacheIndex);
//< Methods and Initializers parse-call
  } else {
...
static void inner_(bool canAssign) {
  if (currentClass == NULL || !currentClass->inMethodBody) {
    error("Can't use 'inner' outside of a class method.");
    return;
  }
  ...
  uint8_t argCount = argumentList();
  emitBytes(OP_INNER_INVOKE, currentClass->currentMethodNameConstant);
  emitByte(argCount);
}
//< Superclasses super
...
static void method() {
  ...
  currentClass->currentMethodNameConstant = constant;
  currentClass->inMethodBody = true;
//> method-body
...
//> method-body
  function(type);
  currentClass->inMethodBody = false;
//< method-body
...
//> Methods and Initializers create-class-compiler
  ...
  classCompiler.inMethodBody = false;
  classCompiler.currentMethodNameConstant = 0;
...
//> Methods and Initializers class-body
  while (!check(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF)) {
    if (match(TOKEN_FIELD)) {
      ...
      uint8_t fieldConst = identifierConstant(&parser.previous);
      emitBytes(OP_FIELD, fieldConst);
