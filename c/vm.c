...
static void bumpMethodMutation(ObjClass* klass) {
  klass->methodVersion++;
  vm.methodEpoch++;
}

static int buildClassChain(ObjClass* leaf, ObjClass** chain, int max) {
  int n = 0;
  for (ObjClass* c = leaf; c != NULL && n < max; c = c->superclass) {
    chain[n++] = c;
  }
  return n;
}

static bool findMethodTopDown(ObjClass* leaf, ObjString* name,
                              Value* outMethod, ObjClass** outOwner) {
  ObjClass* chain[256];
  int n = buildClassChain(leaf, chain, 256);
  for (int i = n - 1; i >= 0; i--) {
    if (tableGet(&chain[i]->methods, name, outMethod)) {
      *outOwner = chain[i];
      return true;
    }
  }
...
static ObjClosure* findTopInitializer(ObjClass* leaf, ObjClass** outOwner) {
  Value initVal;
  if (!findMethodTopDown(leaf, vm.initString, &initVal, outOwner)) {
    return NULL;
  }
  return AS_CLOSURE(initVal);
}
...
static bool call(ObjClosure* closure, int argCount, ObjClass* owner) {
//< Closures call-signature
...
//< store-receiver
        return call(bound->method, argCount, bound->owner);
      }
//< Methods and Initializers call-bound-method
//> Classes and Instances call-class
      case OBJ_CLASS: {
        ...
        ObjClass* initOwner;
        ObjClosure* initializer = findTopInitializer(klass, &initOwner);
        if (initializer != NULL) {
          return call(initializer, argCount, initOwner);
        } else if (argCount != 0) {
...
//> Closures call-value-closure
      case OBJ_CLOSURE:
        return call(AS_CLOSURE(callee), argCount, NULL);
//< Closures call-value-closure
...
static bool invoke(ObjString* name, int argCount, int cacheIndex) {
  CallFrame* frame = &vm.frames[vm.frameCount - 1];
...
//< invoke-field
  ...
  InvokeCacheEntry* cache = &chunk->invokeCaches.entries[cacheIndex];

  Value method;
  ObjClass* owner;
  if (cache->klass == klass && cache->name == name &&
      cache->methodEpoch == vm.methodEpoch &&
      cache->klassVersion == klass->methodVersion) {
    return call(cache->method, argCount, cache->owner);
  }

  if (!findMethodTopDown(klass, name, &method, &owner)) {
    runtimeError("Undefined property '%s'.", name->chars);
    return false;
  }

  cache->klass = klass;
  cache->method = AS_CLOSURE(method);
  cache->owner = owner;
  cache->name = name;
  cache->klassVersion = klass->methodVersion;
  cache->methodEpoch = vm.methodEpoch;
  return call(AS_CLOSURE(method), argCount, owner);
}
//< Methods and Initializers invoke
...
//< Strings concatenate
static bool innerInvoke(ObjString* name, int argCount) {
  CallFrame* frame = &vm.frames[vm.frameCount - 1];
  Value receiver = peek(argCount);
  if (!IS_INSTANCE(receiver)) {
    runtimeError("inner() can only be used on instances.");
    return false;
  }

  ObjInstance* instance = AS_INSTANCE(receiver);
  ObjClass* chain[256];
  int n = buildClassChain(instance->klass, chain, 256);
  ObjClass* owner = frame->ownerClass;
  int ownerIdx = -1;
  for (int i = 0; i < n; i++) {
    if (chain[i] == owner) {
      ownerIdx = i;
  ...
  for (int j = ownerIdx - 1; j >= 0; j--) {
    Value method;
    if (tableGet(&chain[j]->methods, name, &method)) {
      return call(AS_CLOSURE(method), argCount, chain[j]);
    }
  }
...
//> Methods and Initializers interpret-invoke
      case OP_INVOKE: {
        ...
        int argCount = READ_BYTE();
        int cacheIndex = READ_BYTE();
        if (!invoke(method, argCount, cacheIndex)) {
          return INTERPRET_RUNTIME_ERROR;
        }
        frame = &vm.frames[vm.frameCount - 1];
        break;
...
//< Methods and Initializers interpret-invoke
      case OP_INNER_INVOKE: {
        ObjString* method = READ_STRING();
        int argCount = READ_BYTE();
        if (!innerInvoke(method, argCount)) {
          return INTERPRET_RUNTIME_ERROR;
        }
        frame = &vm.frames[vm.frameCount - 1];
        break;
...
//< inherit-non-class
        ...
        tableAddAll(&superclass->declaredFields,
                    &subclass->declaredFields);
        pop(); // Subclass.
        break;
      }
//< Superclasses interpret-inherit
...
//< Methods and Initializers interpret-method
      case OP_FIELD: {
        ...
        Value ignored;
        if (tableGet(&klass->declaredFields, name, &ignored)) {
          runtimeError(
              "Field already declared in this class or superclass.");
          return INTERPRET_RUNTIME_ERROR;
        }
        tableSet(&klass->declaredFields, name, BOOL_VAL(true));
        break;
      }
    }
  }
